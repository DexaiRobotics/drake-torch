...
int main(int argc, char *argv[]) {
  ...
  std::vector<BodyIndex> robot_body_indices =
      plant.GetBodyIndices(robot_model_idx_);
  ...
  Eigen::Vector3d box_cluster_origin{-1.1, 0.8,
                                     0.35}; // meters, arm is at 0,0,0
    ...
    for (size_t y_idx = 0; y_idx < num_boxes(1); y_idx++) {
      for (size_t x_idx = 0; x_idx < num_boxes(0); x_idx++) {
        if (populated_boxes(tot_idx) > 0) {
          ...
          RigidTransform<double> X_WorldToBox(
              drake::math::RollPitchYaw<double>(box_cluster_rpy),
              box_cluster_origin + box_offset.cwiseProduct(grid_offset));
          auto &child_frame = plant.GetFrameByName(box_root_link, box_idx);
