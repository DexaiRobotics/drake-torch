        ...
        if [ $LIBTORCH = true ]; then \
            if [ $BUILD_TYPE = "cpu" ]; then \
                ...
                if [ $BUILD_CHANNEL = "stable" ]; then \
                    ...
                    wget -q https://download.pytorch.org/libtorch/nightly/cu118/libtorch-cxx11-abi-shared-with-deps-latest.zip; \
                fi; \
            fi \
    ...
    && \
        if [ $BUILD_TYPE = "cpu" ]; then \
            ...
            if [ $BUILD_CHANNEL = "stable" ]; then \
                ...
                python3 -m pip install --upgrade --no-cache-dir --compile --pre torch torchvision --index-url https://download.pytorch.org/whl/nightly/cu118; \
            fi; \
        fi
