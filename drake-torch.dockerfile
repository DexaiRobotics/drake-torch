...
RUN set -eux && cd $HOME \
    ...
    && \
        if [ $LIBTORCH = true ]; then \
            if [ $BUILD_TYPE = "cpu" ]; then \
                ...
                if [ $BUILD_CHANNEL = "stable" ]; then \
                    wget -q https://download.pytorch.org/libtorch/cu117/libtorch-cxx11-abi-shared-with-deps-1.13.1%2Bcu117.zip; \
                else \
                    wget -q https://download.pytorch.org/libtorch/nightly/cu117/libtorch-cxx11-abi-shared-with-deps-latest.zip; \
                fi; \
            fi \
    ...
    && \
        if [ $BUILD_TYPE = "cpu" ]; then \
            if [ $BUILD_CHANNEL = "stable" ]; then \
                python3 -m pip install --upgrade --no-cache-dir --compile torch torchvision --extra-index-url https://download.pytorch.org/whl/cpu; \
            else \
            ...
            if [ $BUILD_CHANNEL = "stable" ]; then \
                ...
                python3 -m pip install --upgrade --no-cache-dir --compile --pre torch torchvision --extra-index-url https://download.pytorch.org/whl/nightly/cu117; \
            fi; \
        fi
...
RUN set -eux \
    ...
    && \
        ...
        if [ $BUILD_CHANNEL = "stable" ]; then \
            ...
            && apt install --no-install-recommends -qy ./drake-dev_1.10.0-1_amd64-focal.deb \
